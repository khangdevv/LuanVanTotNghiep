...
from or_tools_generator import solve_schedule  # noqa: E402
...
def print_schedule(idx: int, sched: dict[str, ClassSection]) -> None:
    print(f"\n{'=' * 70}")
    print(f"  TKB #{idx}")
    print(f"{'=' * 70}")
...
def main() -> None:
    print("=" * 70)
    print("  DEMO THUẬT TOÁN XẾP THỜI KHÓA BIỂU — OR-Tools CP-SAT")
    print("=" * 70)
    ...
    print(f"\n[3] Sinh TKB bằng OR-Tools CP-SAT (max {MAX_SOLUTIONS} nghiệm) ...")
    print(f"    Tránh ngày : {sorted(AVOID_DAYS)}")
    t3 = perf_counter()
    schedules = solve_schedule(
    ...
    elapsed_solver = perf_counter() - t3
    print(f"    → OR-Tools CP-SAT: {elapsed_solver:.3f}s")
    ...
    to_print = schedule_scores[:PRINT_TOP]
    print(f"\n[4] In {len(to_print)} TKB tốt nhất:\n")

    for i, (schedule, score) in enumerate(to_print, start=1):
        print(f"\nĐiểm: {score['score_total']:.4f}")
        print(f"  - Break time      : {score['score_break']:.4f}")
        print(f"  - Preference match: {score['score_pref']:.4f}")
        print(f"  - Workload balance: {score['score_balance']:.4f}")
